"""
assistant_core/scripts/train_emotion_model.py
══════════════════════════════════════════════════════════════════════════════
CLI-скрипт для тренування ML-моделі класифікації емоцій.

ЯК ЗАПУСТИТИ
════════════
    # У корені проєкту. Обов’язково інтерпретатор з venv проєкту (у ньому scikit-learn):
    source venv/bin/activate   # macOS / Linux
    python -m assistant_core.scripts.train_emotion_model

    # Або без активації:
    ./venv/bin/python -m assistant_core.scripts.train_emotion_model

    # Якщо `python` не знайдено в shell — на macOS часто є лише `python3`; у venv зазвичай є `python`.
    # Помилка «scikit-learn не встановлено» на `python3` без venv — це системний Python без залежностей.

    # З ключами (опціонально):
    ./venv/bin/python -m assistant_core.scripts.train_emotion_model --test-size 0.25 --output assistant_core/models/my_model.joblib
...
4. Тренує модель, виводить метрики:
        ...
        confusion matrix
5. Зберігає натреновану модель у `assistant_core/models/emotion_model.joblib`
...
# Додаємо корінь проєкту до sys.path, щоб запускалось і як `python assistant_core/scripts/train_emotion_model.py`
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Шлях до файлу збереження моделі (joblib). За замовчуванням — assistant_core/models/emotion_model.joblib.",
    )
    p.add_argument(
        "--min-accuracy",
        type=float,
